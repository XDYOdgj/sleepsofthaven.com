...
$(document).ready(function() {
	...
	initdefault()
	//评论的评分
	let childscore = document.querySelectorAll(".la-star-score")
	$(childscore).click(function() {
		let index = $(childscore).index(this);

		$('.la-star-score').each(function(i, item) {
			if (i <= index) {
...
window.submitComment = () => {
	...
	formData.score = $(".la-star-score.score-active").length;
			...
			html = html + `<div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-12">
									...
									<a href="${item.linkTitle}"> 
										...
										<div class="product-content product-content-padding text-center">
											<h4 class="ellipsis-omit" ><a href="${item.linkTitle}">${item.name}</a></h4>
